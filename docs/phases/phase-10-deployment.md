...
- ~~งานเบื้องหลังที่ค้างเมื่อ container รีสตาร์ต และ retention ของ `email_logs`~~ → ทำแล้วใน [งานต่อเนื่อง](phase-10-followup-recovery-retention.md) · ~~`batch_jobs` / `batch_items` ยังไม่อยู่ในนโยบาย retention~~ → ทำแล้ว (spec v2.3)
