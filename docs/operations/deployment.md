...
| Retention | `scripts/cron-run.sh retention` | ทุกวัน 01:45 | anonymise คำขอ + ลบ Audit Log ที่พ้นกำหนด · ทำเสร็จก่อนตอบกลับ |
...
| ส่งอีเมลซ้ำ | `scripts/cron-run.sh email-retry` | ทุก 15 นาที | retry 4 รอบ (5/30/120/360 นาที) แล้วรอผู้ดูแลกดส่งซ้ำ |
...
- ระหว่างอัปเดต งานแบบชุดหรือ sync ที่กำลังรันอยู่จะถูกตัด — เลือกช่วงเวลาที่ไม่มีงาน (หน้า **ซิงก์ข้อมูล** ไม่มีสถานะ "กำลังรัน")
