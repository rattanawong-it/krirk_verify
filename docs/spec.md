...
> **2026-09-17:** ระบบทะเบียนจริงคือ Keystone Open API → `KeystoneRegistryClient` (`REGISTRY_CLIENT=keystone`) · ดู [`registry-keystone-gap.md`](registry-keystone-gap.md) และ [`phases/phase-11-keystone-registry.md`](phases/phase-11-keystone-registry.md)
...
> บันทึกสรุป Phase 10: [`docs/phases/phase-10-deployment.md`](phases/phase-10-deployment.md) · ปิด F-UX-10 (axe) · งานต่อเนื่อง (กู้คืนงานเบื้องหลังที่ค้าง + retention ประวัติอีเมล): [`docs/phases/phase-10-followup-recovery-retention.md`](phases/phase-10-followup-recovery-retention.md)
...
| 2.2 | 2026-09-16 | ทีมพัฒนา | อนุมัติแล้ว (Phase 10 งานต่อเนื่อง): ค่าตั้งค่า `retention.emailLogDays` (ค่าเริ่มต้น 180 วัน, 30–3650, ต้องไม่นานกว่าระยะเก็บคำขอ) + งาน retention ลบ `email_logs` ที่พ้นกำหนด + นโยบายความเป็นส่วนตัวข้อ 5 ระบุระยะเก็บประวัติอีเมล, cron endpoint `/api/cron/batch-recovery` (ทุก 15 นาที) + audit action `batch.recovered`, งานแบบชุดที่ค้าง PROCESSING เกิน 10 นาทีถูกทำต่ออัตโนมัติโดยผูกแถวกับคำขอที่ยื่นไปแล้ว, `email-retry` ส่งซ้ำอีเมลที่ค้าง PENDING เกิน 30 นาที (response เพิ่ม `recovered`) |
| 2.3 | 2026-09-17 | ทีมพัฒนา | อนุมัติแล้ว (Phase 10 งานต่อเนื่อง — retention งานแบบชุด): งาน retention ลบ `batch_jobs` (+ `batch_items` ตาม FK) ที่สร้างก่อนจุดตัดระยะเก็บคำขอ ยกเว้นสถานะ PROCESSING · งาน DRAFT ที่ไม่ได้ยืนยันถูกลบหลัง 7 วัน (ค่าคงที่ในโค้ด ไม่คืนโควตา) · ไม่เพิ่มค่าตั้งค่า · คำขอที่ยื่นจากไฟล์ไม่ถูกลบ · response/ผลรัน retention เพิ่ม `batchDeleted` · นโยบายความเป็นส่วนตัวข้อ 5 ระบุระยะเก็บไฟล์แบบชุด · หน้ายืนยันงานแบบชุดแจ้งการหมดอายุ |
| 2.4 | 2026-09-17 | ทีมพัฒนา | **รอการอนุมัติ** (เชื่อม Keystone Open API — [`phases/phase-11-keystone-registry.md`](phases/phase-11-keystone-registry.md)): `REGISTRY_CLIENT=keystone` + `KeystoneRegistryClient` (production), กฎ auto-approve ขึ้นกับ `capabilities.councilApprovalDate` (Keystone: สำเร็จการศึกษา + วัน/ภาคที่สำเร็จ + รายละเอียดครบ — กลับไปใช้กฎเดิมเมื่อ API ส่งวันสภาอนุมัติ), แสดงภาคที่สำเร็จเมื่อไม่มีวันสำเร็จการศึกษา + สถานภาพตามข้อความทะเบียนฝั่งเจ้าหน้าที่, sync ข้ามระเบียนด้วย HMAC ของเนื้อหา / หยุดตาม `hasMore` / งานค้างนับจาก `updatedAt`, migration `keystone_registry` |
