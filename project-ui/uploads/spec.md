...
- [ ] **F-SETUP-01** init Next.js 16 (App Router, TypeScript, src dir) ด้วย pnpm
- [ ] **F-SETUP-02** ตั้งค่า Tailwind CSS v4 (CSS-first) + ธีมสีมหาวิทยาลัย + รองรับ dark mode
- [ ] **F-SETUP-03** ติดตั้ง shadcn/ui + component พื้นฐาน (button, input, table, dialog, form, badge, card, select, toast)
- [ ] **F-SETUP-04** ตั้งค่า ESLint + Prettier + TypeScript strict + husky pre-commit
- [ ] **F-SETUP-05** `docker-compose.yml` สำหรับ PostgreSQL (dev) + `.env.example` ครบทุกตัวแปร
- [ ] **F-SETUP-06** ตั้งค่า Prisma + client singleton + script `db:migrate` / `db:seed` / `db:studio`
- [ ] **F-SETUP-07** ตั้งค่า Vitest + Playwright + CI script

### Phase 1 — Authentication & Authorization
- [ ] **F-AUTH-01** Prisma schema: `User`, `Organization` + migration
- [ ] **F-AUTH-02** ตั้งค่า Auth.js v5 (Credentials provider, bcrypt, JWT session, role ใน token)
- [ ] **F-AUTH-03** หน้า Login + validation + error message สองภาษา
- [ ] **F-AUTH-04** หน้าลงทะเบียนหน่วยงานภายนอก (ข้อมูลองค์กร + ผู้ติดต่อ) → สถานะ PENDING
- [ ] **F-AUTH-05** หน้าลงทะเบียนศิษย์เก่า (ยืนยันด้วยรหัสนักศึกษา + เลขบัตรที่ตรงกับข้อมูลใน DB)
- [ ] **F-AUTH-06** ยืนยันอีเมลด้วย token + หน้ายืนยันสำเร็จ
- [ ] **F-AUTH-07** ลืมรหัสผ่าน / รีเซ็ตรหัสผ่านผ่านอีเมล
- [ ] **F-AUTH-08** RBAC helper (`requireRole`, `requireAuth`) + `middleware.ts` ป้องกันทุก route group
- [ ] **F-AUTH-09** เปลี่ยนรหัสผ่าน + หน้าโปรไฟล์ผู้ใช้
- [ ] **F-AUTH-10** นโยบายรหัสผ่าน + ล็อกบัญชีเมื่อล็อกอินผิดเกินกำหนด

### Phase 2 — Data Layer & Registry Integration
- [ ] **F-DATA-01** Prisma schema: `Student` + index ที่จำเป็น (`citizenIdHash`, `passportNoHash`, `studentCode`)
- [ ] **F-DATA-02** โมดูล crypto: `hashIdentifier()` (HMAC-SHA256 + pepper), `encrypt()/decrypt()` (AES-256-GCM), `maskCitizenId()`
- [ ] **F-DATA-03** นิยาม `RegistryClient` interface + Zod DTO ของข้อมูลนักศึกษา
- [ ] **F-DATA-04** `MockRegistryClient` + Mock API route พร้อมข้อมูลตัวอย่าง ≥ 200 ราย ครอบคลุมทุกเคส (จบปกติ, เกียรตินิยม, กำลังศึกษา, ถูกเพิกถอน, ชื่อซ้ำ, ต่างชาติใช้พาสปอร์ต)
- [ ] **F-DATA-05** `HttpRegistryClient` โครงสร้างพร้อมใช้ (retry + timeout + error mapping) รอ spec จริง
- [ ] **F-DATA-06** `sync.service.ts`: full sync + incremental sync + upsert logic + บันทึก `SyncJob`
- [ ] **F-DATA-07** API route `/api/cron/sync` ป้องกันด้วย secret header (เรียกจาก system cron)
- [ ] **F-DATA-08** หน้า Staff `/sync` — ดูประวัติ sync, สถานะล่าสุด, ปุ่ม trigger manual sync
- [ ] **F-DATA-09** ฟังก์ชัน refresh ข้อมูลรายคน (on-demand) จากหน้าพิจารณาคำขอ
- [ ] **F-DATA-10** Seed script สร้างผู้ใช้ตั้งต้นทุกบทบาท + import mock data

### Phase 3 — Core Verification (หัวใจของระบบ)
- [ ] **F-VER-01** Prisma schema: `VerificationRequest`, `VerificationResult` + migration
- [ ] **F-VER-02** Reference No. generator รูปแบบ `KRU-{พ.ศ.}-{running 6 หลัก}` (กัน race condition)
- [ ] **F-VER-03** ฟอร์มยื่นคำขอ: เลือกประเภทคีย์ (บัตรประชาชน/พาสปอร์ต) + ช่องกรอก + วัตถุประสงค์ + PDPA consent (บังคับติ๊ก)
- [ ] **F-VER-04** Validation เลขบัตรประชาชนไทย (checksum 13 หลัก) + รูปแบบพาสปอร์ต
- [ ] **F-VER-05** `verification.service.ts`: matching + กฎ auto-approve (ตามข้อ 5.4)
- [ ] **F-VER-06** สร้าง `VerificationResult` snapshot เมื่ออนุมัติ (immutable)
- [ ] **F-VER-07** หน้าผลตรวจสอบ — แสดงข้อมูลครบตาม R-07 + ตราสัญลักษณ์ + เลขอ้างอิง + วันที่ตรวจสอบ
- [ ] **F-VER-08** Permalink `/verify/result/[refNo]` — เข้าถึงด้วย access token, มีวันหมดอายุ, กันการเดา refNo
- [ ] **F-VER-09** หน้ารายการคำขอของผู้ขอ (filter ตามสถานะ + ค้นหา + แบ่งหน้า)
- [ ] **F-VER-10** Rate limiting ต่อ user และต่อ IP + หน้าแจ้งเตือนเมื่อเกินโควตา
- [ ] **F-VER-11** ป้องกัน enumeration — ไม่เปิดเผยผล "ไม่พบข้อมูล" โดยอัตโนมัติ ให้ผ่านเจ้าหน้าที่เสมอ
- [ ] **F-VER-12** Unit test ครอบคลุมทุกสาขาของกฎ auto-approve / manual-review

### Phase 4 — Registrar Workflow
- [ ] **F-REG-01** หน้าคิวงาน `/queue` — TanStack Table, filter สถานะ/ช่วงวันที่/หน่วยงาน, เรียงตามเวลารอ
- [ ] **F-REG-02** หน้าพิจารณา `/queue/[refNo]` — แสดงข้อมูลคำขอ + รายการที่จับคู่ได้ + เหตุผลที่เข้าคิว
- [ ] **F-REG-03** ค้นหาและเลือกจับคู่นักศึกษาด้วยตนเอง (กรณีระบบหาไม่เจอ/เจอหลายราย)
- [ ] **F-REG-04** อนุมัติคำขอ → สร้าง snapshot + ส่งอีเมลแจ้งผู้ขอ
- [ ] **F-REG-05** ปฏิเสธคำขอ พร้อมเหตุผล (เลือกจากรายการ + กรอกเพิ่ม) + แจ้งผู้ขอ
- [ ] **F-REG-06** บันทึกหมายเหตุภายใน (internal note) ที่ผู้ขอไม่เห็น
- [ ] **F-REG-07** หน้าค้นหาข้อมูลผู้สำเร็จการศึกษา `/students` (เจ้าหน้าที่เท่านั้น) + ดูรายละเอียด
- [ ] **F-REG-08** หน้าจัดการหน่วยงาน `/organizations` — อนุมัติ/ระงับ/ดูประวัติการใช้งาน
- [ ] **F-REG-09** ตัวชี้วัด SLA — แสดงเวลารอของแต่ละคำขอ + ไฮไลต์รายการที่ค้างเกินกำหนด
...
- [ ] **F-AUD-01** Prisma schema: `AuditLog` + `audit.service.ts` (เขียน log แบบ non-blocking)
...
- [ ] **F-UX-01** ตั้งค่า `next-intl` + routing `[locale]` + middleware ตรวจภาษา
- [ ] **F-UX-02** ไฟล์แปล `th.json` / `en.json` ครบทุกข้อความ (ไม่มี hardcoded string)
- [ ] **F-UX-03** ปุ่มสลับภาษาบน header + จำค่าที่ผู้ใช้เลือก
...
- [ ] **F-UX-11** โหลดฟอนต์ไทยที่อ่านง่าย (เช่น Noto Sans Thai / Sarabun) แบบ self-host
